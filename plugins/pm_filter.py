...
from database.connections_mdb import active_connection, all_connections, delete_connection, if_active, make_active, \
    make_inactive
from info import ADMINS, AUTH_CHANNEL, REQ_CHANNEL, AUTH_USERS, SUPPORT_CHAT_ID, CUSTOM_FILE_CAPTION, MSG_ALRT, PICS, AUTH_GROUPS, P_TTI_SHOW_OFF, GRP_LNK, CHNL_LNK, NOR_IMG, LOG_CHANNEL, SPELL_IMG, MAX_B_TN, IMDB, \
        ...
        if glob == False:
            if PM_FILTER_MODE:
                ...
                if total_results == 0:
                    ...
                    msg = await message.reply_text(text=f"<b>Spelling Mistake Bro 🤐, Try Again with correct spelling. </b>", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("Google it 😂 ", url=f"https://www.google.com/search?q={search}_movie")]]))
...
async def language_check(bot, query):
    ...
    if files:
        ...
        if offset != "":
            ...
            btn.append(
                [InlineKeyboardButton(text="Sorry Dear, No more Pages Available ",callback_data="pages")]
            )
        try:
            await query.edit_message_reply_markup(
                reply_markup=InlineKeyboardMarkup(btn)
        ...
        return await query.answer(f"Sorry, No files found with Name :<b>{movie}.</b>", show_alert=True)
...
async def select_language(bot, query):
    ...
    btn = [[
        ...
        InlineKeyboardButton("Telungu", callback_data=f"lang#{userid}#tel")
    ],[
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):
        ...
        try:
            if (AUTH_CHANNEL, REQ_CHANNEL) and not await is_subscribed(client, query):
                if clicked == typed:
                    await query.answer(url=f"https://t.me/{temp.U_NAME}?start={ident}_{file_id}")
                    return
                else:
                    await query.answer(f"എന്റെ മോനെ ഇത് നിനക്ക് ഉള്ളത് അല്ല🤭....", show_alert=True)
            elif settings['botpm']:
                if clicked == typed:
                    await query.answer(url=f"https://t.me/{temp.U_NAME}?start={ident}_{file_id}")
                    return
                ...
                if clicked == typed:
                    if IS_VERIFY and not await check_verification(client, query.from_user.id):
                        ...
                        return await query.answer('File ഞാൻ സ്വകാര്യമായി (PM) അയച്ചിട്ടുണ്ട്.... അവിടെ പോയി നോക്ക് ഫയൽ ഉണ്ട് 😜....', show_alert=True)
                else:
                    return await query.answer(f"എന്റെ മോനെ ഇത് നിനക്ക് ഉള്ളത് അല്ല🤭....", show_alert=True)
        ...
        if file_id == "send_all":
            ...
            if is_over == 'done':
                return await query.answer(f"ഏല്ലാം ആയച്ചിട്ടുണ്ട്😂. PM നോക്ക്", show_alert=True)
            elif is_over == 'fsub':
                return await query.answer("Cʜᴇᴄᴋ ᴍʏ PM ᴛᴏ ᴊᴏɪɴ ᴀɴᴅ ɢᴇᴛ ғɪʟᴇs !", show_alert=True)
            elif is_over == 'verify':
                return await query.answer("Hᴇʏ, Yᴏᴜ ʜᴀᴠᴇ ɴᴏᴛ ᴠᴇʀɪғɪᴇᴅ ᴛᴏᴅᴀʏ. Yᴏᴜ ʜᴀᴠᴇ ᴛᴏ ᴠᴇʀɪғʏ ᴛᴏ ᴄᴏɴᴛɪɴᴜᴇ. Cʜᴇᴄᴋ ᴍʏ PM ᴛᴏ ᴠᴇʀɪғʏ ᴀɴᴅ ɢᴇᴛ ғɪʟᴇs !", show_alert=True)
            else:
...
async def auto_filter(client, msg, spoll=False):
    ...
    btn.insert(0, [
        InlineKeyboardButton("🔃 PM All", callback_data=f"send_fall#{pre}#{0}#{message.from_user.id}"),
        InlineKeyboardButton("Language 🔣", callback_data=f"select_lang#{message.from_user.id}")
    ])
    ...
    if offset != "":
        ...
        btn.append(
            [InlineKeyboardButton(text="No Pages Available",callback_data="pages")]
        )
